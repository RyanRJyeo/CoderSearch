    ...
    if(this.state.loggedIn !== ""){
        if(this.state.loggedIn === "coders"){
            convos = this.state.searchers.map(x=>{
                return  <button key={x.convo_id} value={[x.convo_id, x.details.name, x.details.id]} onClick={(event)=> this.getChat(event)} className={styles.convoName}>{x.details.name}</button>
            });
        } else {
    ...
    allChats = chats.map(x =>{
      if(this.state.loggedIn === "coders"){
        if(x.sender_name === this.state.coders[0].name && x.sender_id === this.state.coders[0].id){
          ...
          return  <div key={x.id} className={styles.sender}>
                    <p className={styles.messageS}>{x.message} <small></small></p>
                  </div>
        } else {
          // IF ITS THE RECEIVER STYLE LIKE THIS
          return  <div key={x.id} className={styles.receiver}>
